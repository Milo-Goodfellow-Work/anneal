...
-- Example illustrating why `assumption` can fail because of recursion depth limits.
-- We keep it commented out to avoid guard-mismatch failures when tooling inspects messages.
...
def extractAssert (ref : Syntax) : TacticM Unit := do
  ...
  let tac : TSyntax `tactic ← `(tactic|have : $goal := by
    -- placeholder
    skip)
