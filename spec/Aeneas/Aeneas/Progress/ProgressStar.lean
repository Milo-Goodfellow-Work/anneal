...
where
  ...
  traverseProgram (cfg : Config): TacticM Info := do
    ...
    | .unknown => do
      trace[Progress] "don't know what to do: giving up"
      let subgoals ← pure ((← getUnsolvedGoals).toArray.map fun g => (g, none))
      let tac ← `(tactic| fail "progress* could not solve the goal")
  ...
  onFinish (mvarId : MVarId) : TacticM (Info × Option MVarId) := do
    ...
    | some mvarId =>
      ...
      let rec tryFinish (tacl : List (String × TacticM Syntax.Tactic)) : TacticM Unit := do
        match tacl with
        | [] =>
          trace[Progress] "could not prove the goal"
          tacStx.resolve (← `(tactic| fail "progress* could not prove the goal"))
        | (name, tac) :: tacl =>
  ...
  onBind (cfg : Config) (varName : Name) : TacticM (Info × Option MVarId) := do
    ...
    if let some {usedTheorem, unassignedVars, preconditions, mainGoal } ← tryProgress cfg then
      ...
      let sorryStx ← `(tactic|· fail "missing precondition proof")
...
Compatibility note (Lean 4.24): this message-guarded example is disabled
because the generated message (and unsolved goals) differ from the original
expectations. Re-enable and adjust if you need to validate it.
-/
-- #guard_msgs in
-- example b (x y : U32) :
