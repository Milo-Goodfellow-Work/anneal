...
async def _submit_to_aristotle(
    ...
    1. Create a description file explaining what we want
    2. Pass Verif.lean and impl files as context
...
-- This file will contain specifications and proofs generated by Aristotle.
...
"""
        ...
        description = f"""Analyze the Lean implementation in Spec.{ctx['name']}.Main and generate formal specifications.

TASK: Write theorems in Spec/{ctx['name']}/Verif.lean that specify the correctness of:
- All public functions and their expected behavior
- Structural invariants for data types
- Edge cases (empty inputs, bounds, etc.)

REQUIREMENTS:
1. All theorems must be fully proven (no sorry)
2. Import Spec.{ctx['name']}.Main to access the definitions
3. Use the namespace Spec.{ctx['name']}
4. Theorems should be meaningful, not trivial

The implementation file Main.lean has already been built and is available in the project context.
"""
        _write_text_file(desc_path, description)
        
        # Context files include the implementation
        context_files = [str(f.relative_to(ctx["spec_pkg_root"])) for f in impl_files]
        
        # For informal mode, use Main.lean as formal_input_context
        main_lean_path = impl_files[0] if impl_files else None
        formal_context = str(main_lean_path.relative_to(ctx["spec_pkg_root"])) if main_lean_path else None
        
        # Submit to Aristotle - DO NOT wait (can take days)
        log(f"Submitting to Aristotle (INFORMAL mode) with {len(context_files)} context files...")
        log(f"Formal context: {formal_context}")
        ...
        result = await aristotlelib.Project.prove_from_file(
            ...
            project_input_type=ProjectInputType.INFORMAL,  # Use INFORMAL mode
            formal_input_context=formal_context,  # Point to Main.lean
            auto_add_imports=True,
            validate_lean_project=True,
            wait_for_completion=False,  # Don't wait - Aristotle can take days
        )
        
        if result:
            log(f"Aristotle job submitted. Project ID: {result}")
            # Return the job ID/path for tracking
